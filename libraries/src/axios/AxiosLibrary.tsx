...
export default function AxiosLibrary() {
    ...
    // 각각의 http method 함수는 결과를 then으로 받을 땐 response 객체를 매개변수로 받는 콜백함수를 전달
    .then(response => {
    ...
    .catch(error => {
        // 해당 request에 대해서 request가 실패하거나 status code가 400, 500번대 이면 catch로 AxiosError 객체를 받아옴
    ...
    axios.get(GET_URL, { 
        headers: { 'Authorization' : 'Bearer 123124124124' }, 
        params: { 'time': '1144' } 
    })
